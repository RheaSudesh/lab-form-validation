...
function checkUserName() {
    ...
    if (usertext == null || username.length < 8 || username.length > 15
        ...
        alert('Please enter valid Username of length 8 to 15 and with no space')
        return false;
    }
    else if (specialChar.test(username)) {
        alert('Please enter valid Username without any special Characters')
        return false;
    }
    else {
        return true;
...
function checkPassword() {
    ...
    if (password.match(/[a-z]/g) && password.match(/[A-Z]/g) && password.match(/[0-9]/g)
        ...
        alert('Please enter valid password with \nAtleast one UpperCase \nAtleast one Lowercase \nAtleast one special Character');
        return true;
    }
}